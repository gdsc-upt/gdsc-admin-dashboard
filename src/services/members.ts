...
export function getApiMembersList(): Promise<MembersResponse[]>{ //this is the promise response of the request
...
export function postApiMember(body: MembersPayload): Promise<MembersResponse[]>{
    const url = baseUrl + 'v1/members';
    return fetchAndParse<MembersResponse[]>(url,{method: "POST",body: JSON.stringify(body) ,headers})
}
